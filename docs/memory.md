...
| `mlp_probe` | `root/evals/mlp_probe.py` | Nonlinear counterpart to `multi_horizon_probe`. Fits MLPs on `state` and `concat_history` with SGD. Decides Poss 1 (linear AR is the ceiling) vs Poss 2 (nonlinear structure exists, our RNN misses it) per `docs/meet2.md` TAKEAWAY. Tier 1 only — `last`/`meanpool` MLPs are Tier 2. |
