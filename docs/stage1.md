...
**Baseline** : linear probe trained on concatenated dino features
...
1. Ablate state representation: cls token vs patches, state dimensionality
2. Ablate recurrent architecture: GRU vs TTT vs Linear TTT
3. Ablate loss: MSE vs contrastive vs combined. Add scene flow auxiliary loss.
4. For each ablation, measure: multi-horizon prediction decay curve (primary), SSv2 linear probe (secondary)
5. Pick best config

### Phase 3: Data scaling

1. Add Kinetics-700 — does more scene diversity improve the multi-horizon decay curve?
2. Train on combined SSv2 + Kinetics-700, re-run evals
3. Test on Epic-Kitchens-100 clips (longer videos) — does the decay curve hold for longer sequences?

### Phase 4: Stress test before stage 2

1. Run reconstruction gap analysis (Eval B) on Epic-Kitchens-100 to quantify information loss
2. Run dynamics probes (Eval C) to verify the state encodes motion, not just appearance
3. Run compression ratio Pareto (Eval F) to find the sweet spot for state size
4. If all look good: freeze stage 1 encoder, move to stage 2
