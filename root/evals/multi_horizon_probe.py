...
- Mean-pool probe   : ||mean(x_0..x_t) @ W_k - x_{t+k}||^2  (unordered multi-frame aggregation)
- State probe       : ||state_t @ W_k_state - x_{t+k}||^2  (linear in encoder state)

The total improvement (copy - state) decomposes additively:
  copy - state = (copy - raw) + (raw - mean) + (mean - state)
                  LIN PROJ      AGGREG         TEMPORAL

- LIN PROJ  : gain from linearly projecting the current frame (static structure).
- AGGREG    : gain from seeing multiple frames without temporal order (denoising).
- TEMPORAL  : gain from temporal ordering — the only part that implies dynamics.

Interpretation
--------------
- state < mean-pool  : temporal ordering contributes beyond unordered aggregation.
- state ≈ mean-pool  : "temporal gain" was just multi-frame averaging, not dynamics.
...
def init_accumulators(max_k, D, device):
    """XtX and XtY accumulators for the state probe and the raw-DINO probe, per horizon."""
    accs = {}
...
def fit_probes(accs, lam_rel):
    """Solve ridge for all horizons, both probes. Returns dicts keyed by horizon k."""
    Ws_state, Ws_raw, Ws_mean = {}, {}, {}
    ...
    return Ws_state, Ws_raw, Ws_mean


@torch.no_grad()
def eval_split(model, feats, Ws_state, Ws_raw, Ws_mean, max_k, batch_size, device):
    """One pass over val: accumulate sum-over-D L2 per horizon for copy / raw / mean / state."""
    N, T = feats.shape[0], feats.shape[1]
    D = feats.shape[-1]

    sums = {k: {"copy": 0.0, "raw": 0.0, "mean": 0.0, "state": 0.0, "n": 0} for k in range(1, max_k + 1)}
    ...
    return {
        k: {m: sums[k][m] / max(sums[k]["n"], 1) for m in ("copy", "raw", "mean", "state")}
        for k in sums
    }
...
def print_results(results, n_train_anchors_per_k):
    ...
    print("=" * 105)
    print(f"{'k':>4s} {'copy':>10s} {'raw':>10s} {'mean-pool':>10s} {'state':>10s} "
          f"{'| TOTAL':>10s} {'LIN PROJ':>10s} {'AGGREG':>10s} {'TEMPORAL':>10s}")
    print(f"{'':>4s} {'':>10s} {'':>10s} {'':>10s} {'':>10s} "
          f"{'| cop-sta':>10s} {'cop-raw':>10s} {'raw-mean':>10s} {'mean-sta':>10s}")
    print("-" * 105)
    for k, row in results.items():
        total    = row["copy"] - row["state"]
        lin_proj = row["copy"] - row["raw"]
        aggreg   = row["raw"]  - row["mean"]
        temporal = row["mean"] - row["state"]
        print(f"{k:>4d} {row['copy']:>10.1f} {row['raw']:>10.1f} {row['mean']:>10.1f} {row['state']:>10.1f} "
              f"{'|':>2s}{total:>8.1f} {lin_proj:>10.1f} {aggreg:>10.1f} {temporal:>10.1f}")
    print()
    # Verdict
    ...
    if beats_mean:
        print("→ State beats unordered multi-frame aggregation — temporal ordering contributes.")
    else:
        print("→ State does NOT beat mean-pool — 'temporal gain' was just multi-frame aggregation.")
...
def main():
    ...
    accs = init_accumulators(args.max_horizon, D, device)
    ...
    Ws_state, Ws_raw, Ws_mean = fit_probes(accs, args.ridge_lambda)
    for k, a in accs.items():
        print(f"  horizon k={k}: fit on {a['n']:,} rows")

    print("\n[3/3] Evaluating on val split...")
    results = eval_split(model, val_feats, Ws_state, Ws_raw, Ws_mean, args.max_horizon,
